...
<body>
<div class="container">
  <h1>Тест — Симулятор (55 питань)</h1>
  ...
  <div class="meta">
    ...
    <div style="width:320px;">
      <label><strong>Електронна пошта для результату:</strong></label>
      <input id="destEmail" type="email" value="chorna.galina69@gmail.com" />
    </div>
  ...
  <footer>
    Після завершення результат автоматично буде надіслано на вказану електронну пошту. Мінімум для проходження — 50% та більше.
  </footer>
</div>

<script>
/* ---------------- ВЕСЬ НАБІР ПИТАНЬ (55) ----------------
   текст питань і варіанти узято/відновлено згідно з документом користувача.
*/
const questions = [
...
function render(){
  ...
  if (state.showResults){
    ...
    const passed = percent >= 50; // 50% і більше проходить
...
function grade(){
  ...
  for (const q of questions){
    if (state.choices[q.id] === answers[q.id]) sc++;
  }
  return sc;
}

/* ---------------- ВІДПРАВКА РЕЗУЛЬТАТІВ ----------------
   Використано FormSubmit (без серверного коду) — запит на formsubmit.co/ajax/[email]
   Якщо хостинг блокує CORS, можна замінити на mailto-посилання або експортувати результат вручну.
*/
function sendEmail(destEmail, text){
  ...
  fetch(url, {
    ...
    // помовчки — результат був відправлений
    console.log('email send status', r.status);
  }).catch(err=>{
    console.warn('Не вдалось надіслати результати автоматично.', err);
  });
}
...
document.getElementById('startBtn').addEventListener('click', ()=>{
  ...
  const dest = document.getElementById('destEmail').value.trim();
  if (!name){
    alert('Будь ласка, введіть ПІБ перед початком тесту.');
    return;
  }
  if (!dest){
    alert('Будь ласка, вкажіть електронну адресу для отримання результатів.');
    return;
  }
  // старт
  state.started = true;
